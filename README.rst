...
* ``--async``
  ...
  * ``--async`` currently works only with ``pype map``, not ``pype apply`` and works only for a single ``map`` command, e.g. ``map 'str.upper || len || ? & 1``, not for chains, e.g. ``map str.upper map len map '? & 1'``.
